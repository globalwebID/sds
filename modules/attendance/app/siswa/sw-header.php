...
$avatarFile = isset($data_user['avatar']) && file_exists('../sw-content/avatar/'.$data_user['avatar']) ? $data_user['avatar'] : '-.jpg';
$kembali = $base_url;
$mod_titles = [
    ...
    'sanksi-pelanggaran' => 'Sanksi Pelanggaran',
];
...
$notifikasi_header = "SELECT * FROM notifikasi WHERE user_id='$data_user[user_id]' AND tujuan='siswa' AND status='N' ORDER BY notifikasi_id DESC LIMIT 5";
$result_header = $connection->query($notifikasi_header);

/** Notifikasi live chat */
$query_notifikasi_chat ="SELECT chat_id FROM chat WHERE status_pegawai='N' AND user_id='$data_user[user_id]'";
