...
if(!isset($_COOKIE['ADMIN_KEY']) && !isset($_COOKIE['KEY'])){
  ...
  exit;
    ...
    if(!empty($_POST['kelas_id'])){
        $kelas_id = convert('decrypt', $_POST['kelas_id']??'-');
    }else{
        $kelas_id ='-';
    }
    ...
    $sWhere = "WHERE parent_id='$kelas_id'";
    ...
    while ($aRow = mysqli_fetch_array($rResult)){$no++;
        ...
        for ($i=1 ; $i<count($aColumns) ; $i++){
            ...
            $query_kelas="SELECT nama_kelas FROM absensi_kelas WHERE kelas_id='$aRow[parent_id]'";
            $result_kelas = $connection->query($query_kelas);
            if(!empty($result_kelas && $result_kelas->num_rows > 0)){
                $data = $result_kelas->fetch_assoc();
                $tingkatan = ($data['nama_kelas']??'-');
            }else{
                $tingkatan ='Belum ada kelas';
            }

            if($modifikasi =='Y'){
                $btn_update = '<a href="javascript:void(0)" data-id="'.htmlentities(convert('encrypt', $aRow['kelas_id']??'-')).'" class="table-action table-action-primary btn-update" data-toggle="tooltip" title="Edit">
            ...
            if($hapus =='Y'){
                $btn_hapus ='<a href="javascript:void(0)" class="table-action table-action-delete btn-tooltip btn-delete-kelas" data-toggle="tooltip" data-id="'.htmlentities(convert('encrypt', $aRow['kelas_id']??'-')).'" title="Hapus">
